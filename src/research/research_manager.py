...
from ..feedback.answer_evaluator import AnswerEvaluator
from .question_handler import QuestionHandler
from .answer_generator import AnswerGenerator
from ..storage.file_storage import FileStorage
from .research_parallel import ResearchParallel

class ResearchManager:
    """
    건강기능식품 연구 프로세스 관리 클래스
    ...
    async def load_questions(self, questions_file: str = None, questions: List[str] = None) -> List[str]:
        """
        연구 질문 로드
    ...
    async def run_research(self, questions: List[str] = None, questions_file: str = None) -> Dict[str, Any]:
        """
        질문 목록에 대한 연구 진행
        ...
        print(f"===== 건강기능식품 연구 시작 ({len(research_questions)}개 질문) =====")
    ...
    async def research_question(self, question: str, question_id: str = None) -> Dict[str, Any]:
        """
        단일 질문에 대한 연구 수행
        ...
        try:
            ...
            template = "다음 질문에 대해 근육 관련 건강기능식품 전문가로서 과학적 근거와 참고문헌을 포함하여 상세히 답변해주세요:\n\n{question}"
            system_prompt = """당신은 근육 관련 건강기능식품 전문가입니다. 다음 형식으로 답변하세요:
            1. 문제 정의
            2. 핵심 내용 (이론, 개념, 원리)
            3. 과학적 근거 (연구 결과, 데이터)
            4. 복용 방법 및 주의사항
    ...
    async def conduct_research(self, questions: List[str], concurrent_limit: int = None) -> Dict[str, Any]:
        """
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='근육 관련 건강기능식품 연구 시스템')
